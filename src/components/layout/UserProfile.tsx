...
export function UserProfile({ onOpenSettings }: { onOpenSettings: () => void }) {
  ...
  return (
    <div className="h-28 px-6 border-t border-white/5 bg-zinc-950 shrink-0 flex items-center justify-center shadow-2xl" style={{ WebkitAppRegion: "no-drag" } as any}>
      <div 
      ...
      >
        <div className="flex items-center gap-3 truncate">
          {user?.photoURL ? (
            <img src={user.photoURL} alt="Profile" className="w-8 h-8 rounded-sm shrink-0 object-cover" />
          ) : (
        ...
        <button className="text-white hover:text-white/80 transition-colors p-2 bg-white/5 hover:bg-white/10 rounded-sm shrink-0">
