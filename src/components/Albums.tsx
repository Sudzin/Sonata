...
export function AlbumsView() {
  ...
  return (
    <div className="p-8 h-full overflow-y-auto custom-scrollbar">
      <h1 className="text-3xl font-bold tracking-tight text-white mb-8">{t.albums}</h1>
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-6 pb-24">
        {albums.map(([albumName, tracks]) => {
          ...
          return (
            <div 
              key={albumName}
              className="bg-white/5 rounded-2xl p-4 border border-white/5 hover:bg-white/10 transition-colors group cursor-pointer"
              onClick={() => playTrack(tracks[0], tracks)}
            >
              <div className="aspect-square rounded-xl overflow-hidden bg-white/10 relative shadow-lg mb-4">
                ...
                <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                  <button className="w-12 h-12 rounded-full bg-emerald-400 text-black flex items-center justify-center hover:scale-110 transition-transform shadow-xl">
                    <Play className="w-6 h-6 fill-current ml-1" />
                  </button>
                </div>
              </div>
              <h3 className={`font-bold truncate ${isPlayingFromAlbum ? 'text-emerald-400' : 'text-white'}`}>{albumName}</h3>
              <p className="text-white/50 text-sm truncate">{coverTrack?.artist}</p>
